...
import com.erge.componentization.ext.AppLikeExtensions
import com.erge.componentization.ext.AppLikeManager
...
class LifecycleTransform extends Transform {
    public static String SCAN_INTERFACE
    public static String SCAN_MANAGER
    public static AppLikeManager mgr
    public static final HashSet<String> LIST = new HashSet<>()
    /**
     * com/erge/lifecycle/AppLifecycleManager所在的jar包
     */
    public static File manager_jar

    private static AppLikeExtensions ext
    ...
    boolean initOk() {
        ext = project.extensions.getByType(AppLikeExtensions)
        try {
            ext.checkNotNull()
        } catch (Exception e) {
            Log.e e
        ...
        ext.normalize()
        SCAN_INTERFACE = ext.appLikeInterface
        mgr = ext.appLikeMgr
        SCAN_MANAGER = mgr.name
        Log.i "register for ${mgr.name}#${mgr.register}(Ljava/lang/String;)V"
        Log.i "init for ${mgr.name}#${mgr.init}()V"
        true
    }
    ...
    void transform(Context context, Collection<TransformInput> inputs, Collection<TransformInput> referencedInputs,
        ...
        inputs.each { input ->
            ...
            input.jarInputs.each { jarInput ->
                ...
                while (ok && entries.hasMoreElements()) {
        ...
        if (!ok || manager_jar == null || !manager_jar.exists() || LIST.isEmpty()) {
            return
        }
        println """\
        ...
        void visitInsn(int opcode) {
            if (opcode >= IRETURN && opcode <= RETURN) {
                LIST.each { name ->
                    ...
                    mv.visitMethodInsn(INVOKESTATIC, SCAN_MANAGER, ext.appLikeMgr.register, "(Ljava/lang/String;)V", false)
                }
                mv.visitMethodInsn(INVOKESTATIC, SCAN_MANAGER, ext.appLikeMgr.init, "()V", false)
            }
