...
import com.erge.componentization.ext.Config
...
class ComponentPlugin implements Plugin<Project> {
    private static final String prefix = 'com.component'
    ...
    void apply(Project project) {
        if(project.plugins.hasPlugin(ComponentPlugin)){
            return
        }
        project.extensions.create('appLike', AppLikeExtensions)
        // 模块没有指定$KEY_MAIN_MODULE直接退出, 不影响模块
        if (!project.hasProperty(KEY_RUNNING_ALONE)) {
            throw new NoSuchPropertyException("no such property '$KEY_RUNNING_ALONE' found under ${project.path}")
        }
        ...
        if (project.rootProject.hasProperty(KEY_MAIN_MODULE)) {
            ...
            Log.e "没有在${project.rootProject.name}/gradle.properties中配置 property '${KEY_MAIN_MODULE}'" +
        ...
        // 判断当前模块是否支持独立运行
        boolean isRunningAlone = Boolean.parseBoolean(project.property(KEY_RUNNING_ALONE) as String)
        ...
        project.extensions.create('config', Config).isApp = isRunningAlone
        ...
        if (isRunningAlone) {
            ...
            //class转换
            if(project.extensions.getByType(AppLikeExtensions)) {
                def android = project.extensions.getByType(AppExtension)
                android.registerTransform(new LifecycleTransform(project))
            }else{
                Log.e "${project.path}不使用生命周期注册"
            }
        } else {
